...
public class Url extends CollectionElement implements JsonDeserializer<Url> {
    private String raw = "";
     ...
     */
    public void addPathVariable(String key, String value, String description) {
     ...
     */
    public void addPathVariable(Property varPath) {
    ...
    public Url deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context) throws JsonParseException {
        try {
            if (json.isJsonObject()) {
                ...
                Url url = new Url();
                if (jsonObject.has("raw") && !jsonObject.get("raw").isJsonNull()) {
                    url.raw = jsonObject.get("raw").getAsString();
                }

                if (jsonObject.has("host") && !jsonObject.get("host").isJsonNull()) {
                    url.host = context.deserialize(jsonObject.get("host"), ArrayList.class);
                }

                if (jsonObject.has("path") && !jsonObject.get("path").isJsonNull()) {
                    url.path = context.deserialize(jsonObject.get("path"), ArrayList.class);
                }

                if (jsonObject.has("query") && !jsonObject.get("query").isJsonNull()) {
                    url.query = context.deserialize(jsonObject.get("query"), PropertyList.class);
                }

                if (jsonObject.has("variable") && !jsonObject.get("variable").isJsonNull()) {
                    url.variable = context.deserialize(jsonObject.get("variable"), PropertyList.class);
                }

                if (jsonObject.has("protocol") && !jsonObject.get("protocol").isJsonNull()) {
                    url.protocol = jsonObject.get("protocol").getAsString();
                }

                if (jsonObject.has("port") && !jsonObject.get("port").isJsonNull()) {
                    url.port = jsonObject.get("port").getAsString();
                }
                return url;
